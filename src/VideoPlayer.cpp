...
VideoPlayer::VideoPlayer(HWND hwnd)
    : m_hwndVideo(hwnd), m_isPlaying(false), m_nRefCount(1), m_pSession(NULL),
      m_pSource(NULL), m_pVideoDisplay(NULL) {}
...
HRESULT VideoPlayer::HandleEvent(IMFMediaEvent *pEvent) {
  ...
  } else if (meType == MESessionTopologyStatus) {
    UINT32 status = 0;
    if (SUCCEEDED(pEvent->GetUINT32(MF_EVENT_TOPOLOGY_STATUS, &status))) {
      if (status == MF_TOPOSTATUS_READY) {
        // Topology 已經準備好，此時 MFGetService(MR_VIDEO_RENDER_SERVICE)
        // 才拿得到
        if (m_hwndVideo) {
          RECT rc;
          GetClientRect(m_hwndVideo, &rc);
          ResizeVideo(rc.right - rc.left, rc.bottom - rc.top);
          Play(); // Topology 就緒後才開始播放
          LogMsg("Topology READY! Video is now PLAYING!");
        }
      }
    }
  }
...
HRESULT VideoPlayer::CreateMediaSource(const std::wstring &url,
...
HRESULT VideoPlayer::CreateTopology(IMFMediaSource *pSource, HWND hVideoWindow,
...
HRESULT VideoPlayer::AddToTopology(IMFTopology *pTopology,
                                   IMFMediaSource *pSource, DWORD streamIndex,
                                   HWND hwndVideo) {
...
HRESULT VideoPlayer::ResizeVideo(WORD width, WORD height) {
  ...
  if (m_pVideoDisplay) {
    ...
    // To fill the screen and crop instead of letterbox, we can set AR mode and
    // Pan&Scan.
    m_pVideoDisplay->SetAspectRatioMode(MFVideoARMode_PreservePicture);

    // This makes it fill the destination RECT without stretching, cropping the
    // overflow. It requires the video to support Pan & Scan, but setting the
    // destination rect does the trick.
    RECT rcDest = {0, 0, width, height};

    // For true "Fill screen and crop edges" (similar to object-fit: cover in
    // CSS): We can let EVR preserve picture, but we would need to calculate the
    // crop ourselves to map to the normalized source rect. However, a simple
    // MFVideoARMode_PreservePicture | MFVideoARMode_NonLinearStretch is not
    // crop. Wait, EVR's default "letterbox" is not what the user wants. The
    // user wants "fill the screen". Let's manually calculate a normalized
    // SrcRect to achieve "Crop to Fill".

    // Let's get the native video size first
    SIZE nativeSize = {0, 0};
    SIZE arSize = {0, 0};
    if (SUCCEEDED(m_pVideoDisplay->GetNativeVideoSize(&nativeSize, &arSize)) &&
      ...
      // Tell EVR not to letterbox, just paint our cropped source rect to the
      // full dest rect
...
HRESULT VideoPlayer::SetVideoWindow(HWND hwnd) {
  // 如果 m_pVideoDisplay 還沒建立，先取得它
  if (!m_pVideoDisplay && m_pSession) {
    MFGetService(m_pSession, MR_VIDEO_RENDER_SERVICE,
                 IID_PPV_ARGS(&m_pVideoDisplay));
  }
  ...
  if (m_pVideoDisplay) {
    m_hwndVideo = hwnd;
    HRESULT hr = m_pVideoDisplay->SetVideoWindow(hwnd);
    return hr;
  }
  // 如果 session 還沒建立好，先記下來，之後 ResizeVideo 時會使用 m_hwndVideo
  m_hwndVideo = hwnd;
  return S_FALSE;
}
